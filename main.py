...
def parse_auctions(url):
    ...
    auction_ended = response.html.find('div.row.auction-bidbar > div.col.width-constraint > div > div > ul > li.time > span > span', first=True).text
    sale_details = response.html.find('div.row.auction-bidbar > div.col.width-constraint > div > div > ul > li.ended > span.value', first=True).text
    if not year.isdigit():
        year = None
    ...
    if 'Auction Cancelled' in sale_details:
        ...
        if 'bid' in sale_details.lower() or 'sold after' in sale_details.lower():
            ...
            sold_or_bid_to = None
        bids = response.html.find('div.row.auction-bidbar > div.col.width-constraint > div > div > ul > li.num-bids > span.value', first=True).text
        comments = response.html.find('div.row.auction-bidbar > div.col.width-constraint > div > div > ul > li.num-comments > span.value', first=True).text
    ...
    try:
        ...
        try:
            ...
            # print(f'Prob no zip code provided. Error: {e}')
            state = location.split(',')[1].split()[0]
    ...
    details = (link, title, bids, comments, auction_ended, sold_or_bid_to, price, seller, seller_type, year, make, model,
               transmission, drivetrain, body_style, mileage, state, city, zip_code, province)
    return details
...
for x in range(145, 156):
    ...
    if auctions_on_page:
        ...
        print('wrote auction to database. moving on to next page...')
